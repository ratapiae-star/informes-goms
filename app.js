...
async function renderConfig(){
  ...
  <div class="screen">
    ...
    <div class="field">
      ...
      <input type="text" id="cfgCorreo" placeholder="calidad@empresa.cl, jefe.terreno@empresa.cl" value="${escapeHtml(c.destinoCorreo)}">
      <div class="hint">Puedes escribir varios correos separados por coma. Se incluyen como destinatarios sugeridos en cada envío automático.</div>
    </div>
    <div class="field">
      <label>Webhook / endpoint (opcional)</label>
      <input type="text" id="cfgWebhook" placeholder="https://tu-flujo.cl/recibir-informe" value="${escapeHtml(c.webhookUrl)}">
...
function laborCardHtml(l, idx, t, catalog, numCounter){
  const fotos = l.fotos.map((f, fi) => {
    const url = fotoUrl(f), cat = fotoCat(f);
    return `
...
function bindDesarrollo(id){
  ...
  document.querySelectorAll('[data-photocat]').forEach(sel => sel.addEventListener('change', async () => {
    ...
    r.labores[idx].fotos[fi] = { url: fotoUrl(f), cat: sel.value };
    r.updatedAt = Date.now();
    await idbPut('reports', r);
  }));
...
function buildPdfBlob(r, cfg){
  ...
  r.labores.forEach((l, li) => {
    ...
    CAT_KEYS.forEach(cat => {
      ...
      doc.text(`${CAT_LABELS[cat]} (${fotos.length})`, margin, y);
      y += 14;
      fotos.forEach((f, fi) => {
        ...
        try { doc.addImage(fotoUrl(f), 'JPEG', x, yy, cellW, cellH, undefined, 'FAST'); } catch(e){ /* ignore malformed image */ }
      });
      const rows = Math.ceil(fotos.length/2);
      y += rows*(cellH+gap) + 6;
    });
  });

  // ---- Firmas — continue right after existing content when there's room, instead of
  // always forcing a fresh page (avoids leaving a mostly-blank page behind) ----
  ...
  const legalLines = doc.splitTextToSize(legalNote, contentW);
  const firmasBlockH = 30 + boxH + 14 + legalLines.length*10;
  ...
  y += boxH + 14;
  doc.setFont('helvetica','italic'); doc.setFontSize(7.5); doc.setTextColor(COL.dim);
